<?php
    ...
    */
    ...
    function atelier_blog_classes( $blog_type, $fullwidth = "no" ) {
        global $atelier_sidebar_config;
        ...
        if ( $blog_type == "mini" ) {
            ...
            if ( $atelier_sidebar_config == "both-sidebars" ) {
                $item_class = "col-sm-3";
            } else {
                $item_class = "col-sm-4";
            }
    ...
    /* BLOG ITEMS
    ================================================== */
    if ( ! function_exists( 'atelier_blog_items' ) ) {

        function atelier_blog_items( $atts ) {
            ...
            if ( $blog_type == "masonry" ) {
                ...
                // Thumb Type
                if ( $hover_style == "default" && function_exists( 'atelier_get_thumb_type' ) ) {
                ...
                if ( $atelier_sidebar_config == "no-sidebars" ) {
                    $wrap_class .= apply_filters('atelier_timeline_blog_nosidebars_wrap_class', 'col-sm-8 col-sm-offset-2');
                }
            } else {
            ...
            while ( $blog_items->have_posts() ) : $blog_items->the_post();
                ...
                $blog_items_output .= atelier_get_post_item( $post->ID, $blog_type, $show_title, $show_excerpt, $show_details, $excerpt_length, $content_output, $show_read_more, $fullwidth );
            ...
            if ( $social_integration == "yes" && $pagination == "none" ) {
                ...
                if ( $twitter_username != "" ) {
                    if ( $fullwidth == "yes" ) {
                        $blog_items_output .= '<ul class="blog-tweets">' . atelier_get_tweets( $twitter_username, $tweet_count, 'blog-fw', $item_class ) . '</ul>';
                    } else {
                        $blog_items_output .= '<ul class="blog-tweets">' . atelier_get_tweets( $twitter_username, $tweet_count, 'blog', $item_class ) . '</ul>';
                    }
                }
                ...
                if ( $instagram_id != "" && $instagram_token != "" ) {
                    $blog_items_output .= '<ul class="blog-instagrams" data-title="' . __( "Instagram", 'atelier' ) . '" data-count="' . $instagram_count . '" data-userid="' . $instagram_id . '" data-token="' . $instagram_token . '" data-itemclass="' . $item_class . '"></ul>';
                }
            }
            ...
            if ( $pagination == "infinite-scroll" ) {
                ...
                $blog_items_output .= '<a href="#" class="load-more-btn">' . __( 'Load More', 'atelier' ) . '</a>';
    ...
    /* BLOG AUX
    ================================================== */
    if ( ! function_exists( 'atelier_blog_aux' ) ) {
        function atelier_blog_aux( $width ) {

            $blog_aux_output = "";
            global $atelier_options;
            $rss_feed_url = __( $atelier_options['rss_feed_url'], 'atelier' );
            ...
            // CATEGORIES
            $blog_aux_output .= '<li><a href="#" class="blog-slideout-trigger" data-aux="categories"><i class="ss-index"></i>' . __( "Categories", 'atelier' ) . '</a>';

            // TAGS
            $blog_aux_output .= '<li><a href="#" class="blog-slideout-trigger" data-aux="tags"><i class="ss-tag"></i>' . __( "Tags", 'atelier' ) . '</a>';

            // SEARCH FORM
            $blog_aux_output .= '<li class="search"><form method="get" class="search-form" action="' . home_url() . '/">';
            $blog_aux_output .= '<input type="text" placeholder="' . __( "Search", 'atelier' ) . '" name="s" />';
            $blog_aux_output .= '</form></li>';

            // ARCHIVES
            $blog_aux_output .= '<li><a href="#" class="blog-slideout-trigger" data-aux="archives"><i class="ss-storagebox"></i>' . __( "Archives", 'atelier' ) . '</a>';

            // RSS LINK
            if ( $rss_feed_url != "" ) {
                $blog_aux_output .= '<li><a href="' . $rss_feed_url . '" class="rss-link" target="_blank"><i class="fas fa-rss"></i>' . __( "RSS", 'atelier' ) . '</a>';
            }

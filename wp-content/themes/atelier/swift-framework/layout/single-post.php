<?php
    ...
    /* META SETUP */
    $show_author_info        = atelier_get_post_meta($post->ID, 'sf_author_info', true );
    $show_social             = atelier_get_post_meta($post->ID, 'sf_social_sharing', true );
    $show_related            = atelier_get_post_meta($post->ID, 'sf_related_articles', true );
    ...
    /* PAGE BUILDER CHECK */
    $page_content_class = apply_filters( 'atelier_post_page_content_class', 'container' );
    ...
    if ( $pb_active != "true" || ( $pb_active == "true" && $sidebar_config != "no-sidebars" ) ) {
        ...
        if ( $page_design_style == "hero-content-split" ) {
        	$content_wrap_class = apply_filters( 'atelier_post_content_wrap_class_content_split', 'col-sm-12' );
        	$post_aux_wrap_class = apply_filters( 'atelier_post_aux_wrap_class', 'col-sm-12' );
        } else if ( $sidebar_config == "right-sidebar" ) {
        	if ($atelier_options['sidebar_width'] == "reduced") {
				$content_wrap_class = apply_filters( 'atelier_post_content_wrap_class_rightsidebar', 'col-sm-9 content-left' );
        	} else {
        		$content_wrap_class = apply_filters( 'atelier_post_content_wrap_class_rightsidebar', 'col-sm-8 content-left' );
        	}
        } else if ( $sidebar_config == "left-sidebar" ) {
			if ($atelier_options['sidebar_width'] == "reduced") {
				$content_wrap_class = apply_filters( 'atelier_post_content_wrap_class_rightsidebar', 'col-sm-9 content-right' );
			} else {
				$content_wrap_class = apply_filters( 'atelier_post_content_wrap_class_rightsidebar', 'col-sm-8 content-right' );
			}
        } else {
            $content_wrap_class = apply_filters( 'atelier_post_content_wrap_class_nosidebar', 'col-sm-8 col-sm-offset-2' );
        }
    } else {
    	if ( $page_design_style == "hero-content-split" ) {
    		$content_wrap_class = apply_filters( 'atelier_post_content_wrap_class_content_split', 'col-sm-12' );
    		$post_aux_wrap_class = apply_filters( 'atelier_post_aux_wrap_class', 'col-sm-12' );
       		...
       		$content_wrap_class = apply_filters( 'atelier_post_content_wrap_class_content_pb_active', '' );
       		$post_aux_wrap_class = apply_filters( 'atelier_post_aux_wrap_class', 'col-sm-8 col-sm-offset-2' );
       	}
    }
    ...
    if ( $fw_media_display == "fw-media-title" && $media_type != "none" ) {
        remove_action( 'atelier_post_article_start', 'atelier_post_detail_heading', 10 );
    }
    if ( $sidebar_config == "one-sidebar" || $fw_media_display == "standard" ) {
        remove_action( 'atelier_post_article_start', 'atelier_post_detail_media', 20 );
        add_action( 'atelier_post_content_start', 'atelier_post_detail_media', 10 );
    }

    /* CUSTOM FORMATTING */
    $extra_paragraph_spacing = atelier_get_post_meta($post->ID, 'sf_extra_paragraph_spacing', true );
    ...
    if ( ! $show_related ) {
        ...
        remove_action( 'atelier_post_after_article', 'atelier_post_related_articles', 30 );
        remove_action( 'atelier_post_after_article_extras', 'atelier_post_related_articles', 10 );
    }
    ...
    /* REVIEW */
    $review_post = atelier_get_post_meta($post->ID, 'sf_review_post', true );
    if ( $review_post ) {
        add_action( 'atelier_post_content_end', 'atelier_post_review', 20 );
    }
?>

<?php while (have_posts()) : the_post(); ?>

    <?php do_action( 'atelier_post_before_article' ); ?>
    ...
    <?php do_action( 'atelier_post_after_article_extras' ); ?>

        ...
        class ReduxFramework_extension_custom_icon_fonts {
                    ...
                    foreach ( $fonts as $section ) {
                        if ( $section['type'] == "d" && ! empty( $section['name'] ) ) {
                            if ( $section['name'] == "custom" ) {
                                $section['name'] = __( 'Custom Icon Fonts', 'atelier' );
                            } else if ( $section['name'] == "fontsquirrel" ) {
                                $section['name'] = __( 'Fonts Squirrel', 'atelier' );
                            }
            ...
            public function ajax() {
                ...
                if ( isset( $_REQUEST['type'] ) && $_REQUEST['type'] == "delete" ) {
                    if ( $_REQUEST['section'] == __( 'Custom Icon Fonts', 'atelier' ) ) {
                        $_REQUEST['section'] = "custom";
                    }
                    
                    if ( $_REQUEST['section'] == __( 'Fonts Squirrel', 'atelier' ) ) {
                    ...
                    try {
                        ...
                        if ( $name_pos >= 0 ) {     
                            delete_option('atelier_fontello_icon_codes');
                        ...
                        echo json_encode( array(
                            'type' => 'error',
                            'msg'  => __( 'Unable to delete font file(s).', 'atelier' )
                        ) );
                    }
            ...
            function processWebfont( $attachment_id, $name, $mime_type, $subfolder = 'custom/' ) {
                ...
                if ( empty( $path ) ) {
                    echo json_encode( array(
                        'type' => 'error',
                        'msg'  => __( 'Attachment does not exist.', 'atelier' )
                    ) );
                ...
                if ( $subtype == "zip" ) {
                    ...
                    if ( $name_pos >= 0 ) {     
                        ...
                        $fontello_data = json_decode( $wp_filesystem->get_contents(  $fontello_path_file ) );
                        
                        foreach ($fontello_data->glyphs as $icon_key){
                                array_push($fontello_codes , $icon_key->css);
                        }

                        update_option('atelier_fontello_icon_codes', $fontello_codes) ;
                         ...
                         if ( ! is_dir( $this->upload_dir . '/fontello_css/'  ) ) {
                                ...
                                if (!copy($fontello_css_path_file, $this->upload_dir . '/fontello_css/fontello-embedded.css' ) ) {
                                   echo json_encode( array(
                                        'type' => 'error',
                                        'msg'  => __( 'Error when creating Fontello file.', 'atelier' )
                                        ) ); 
                                    }
                            }
                    }
                    ...
                    echo json_encode( array(
                        'type' => 'error',
                        'msg'  => __( 'File type not recognized.', 'atelier' )
                    ) );
                ...
                if (is_array($ret) && !empty($ret)) {
                    $msg = __("Unidentified error.", 'atelier');
            ...
            private function getMissingFiles( $name, $fontname, $missing, $output, $subfolder ) {
                ...
                if ( count( $output ) == 1 && isset( $output['eot'] ) ) {
                    echo json_encode( array(
                        'type' => 'error',
                        'msg'  => __( 'The font format .eot is not supported.', 'atelier' )
                    ) );
                ...
                if ( ! isset( $main ) ) {
                    echo json_encode( array(
                        'type' => 'error',
                        'msg'  => __( 'No valid font file was found.', 'atelier' )
                    ) );
                ...
                foreach ( $missing as $idx => $item ) {
                    ...
                    if ( $output[ $main ] ) {
                        ...
                        $payload .= "\r\n";
                        $payload .= $wp_filesystem->get_contents( $output[ $main ] );
                        $payload .= "\r\n";
                    }
                    ...
                    if( is_wp_error( $response ) ) {
                        return array(
                            'type' => 'error',
                            'msg'  => __( 'Your font could not be converted at this time. Please try again later.', 'atelier' )
                        );
            ...
            function add_section () {
                if ( !isset ( $this->parent->fontControl ) ) {
                    $this->parent->sections[] = array(
                        'title' => __ ( 'Font Control', 'atelier' ),
                        'desc' => __ ( '<p class="description"></p>', 'atelier' ),
                        'icon' => 'el-icon-font',
                    ...
                    for ( $i = count ( $this->parent->sections ); $i >= 1; $i -- ) {
                        if ( isset ( $this->parent->sections[ $i ] ) && isset ( $this->parent->sections[ $i ][ 'title' ] ) && $this->parent->sections[ $i ][ 'title' ] == __ ( 'Font Control', 'atelier' ) ) {

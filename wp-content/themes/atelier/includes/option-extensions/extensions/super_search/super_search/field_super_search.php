    ...
    class ReduxFramework_super_search extends ReduxFramework {
       ...
       public function render () {
            ...
            $defaults = array(
                ...
                'content_title' => __ ( 'Filter', 'atelier' )
            );

            $this->field = wp_parse_args ( $this->field, $defaults );

            echo '<div class="redux-supersearch-accordion" data-new-content-title="' . esc_attr ( sprintf ( __ ( 'New %s', 'atelier' ), $this->field[ 'content_title' ] ) ) . '">';
            ...
            if ( isset ( $this->value ) && is_array ( $this->value ) && !empty ( $this->value ) ) {
                ...
                foreach ( $slides as $slide ) {
                    ...
                    //BEFORE TEXT
					$placeholder = ( isset ( $this->field[ 'placeholder' ][ 'beforetext' ] ) ) ? esc_attr ( $this->field[ 'placeholder' ][ 'beforetext' ] ) : __ ( 'Before Text', 'atelier' );
                    echo '<li><input type="text" id="' . $this->field[ 'id' ] . '-before_text_' . $x . '" name="' . $this->field[ 'name' ] . '[' . $x . '][beforetext]' . $this->field['name_suffix'] . '" value="' . esc_attr ( $slide[ 'beforetext' ] ) . '" placeholder="' . $placeholder . '" class="full-text slide-title" /></li>';
              		...
              		$filter_array = atelier_get_woo_product_filters_array();
					
              		//FILTER 
			        $placeholder = ( isset ( $this->field[ 'placeholder' ][ 'ss-filter' ] ) ) ? esc_attr ( $this->field[ 'placeholder' ][ 'ss-filter' ] ) : __ ( 'Filter', 'atelier' );
					...
					echo '<option value="">'.__ ( 'Choose an option', 'atelier' ).'</option>';
					...
					//LABEL
			        $placeholder = ( isset ( $this->field[ 'placeholder' ][ 'label' ] ) ) ? esc_attr ( $this->field[ 'placeholder' ][ 'label' ] ) : __ ( 'Label', 'atelier' );
					...
					//AFTER TEXT
			        $placeholder = ( isset ( $this->field[ 'placeholder' ][ 'aftertext' ] ) ) ? esc_attr ( $this->field[ 'placeholder' ][ 'aftertext' ] ) : __ ( 'After Text', 'atelier' );
                    ...
                    echo '<li><a href="javascript:void(0);" class="button deletion redux-supersearch-remove">' . sprintf ( __ ( 'Delete %s', 'atelier' ), $this->field[ 'content_title' ] ) . '</a></li>';
            ...
            if ( $x == 0 ) {
                ...
                echo '<img class="redux-supersearch-image" id="image_image_id_' . $x . '" src="" alt="' . __('Supersearch Image', 'atelier') . '" target="_blank" rel="external" />';
                echo '</a>';
                echo '</div>';
			...
			/* Label Field */
				...
				/*   After Text Field */
				$placeholder = ( isset ( $this->field[ 'placeholder' ][ 'after' ] ) ) ? esc_attr ( $this->field[ 'placeholder' ][ 'after' ] ) : __ ( 'After Text', 'atelier' );
            ...
            echo '</div><a href="javascript:void(0);" class="button redux-supersearch-add button-primary" rel-id="' . $this->field[ 'id' ] . '-ul" rel-name="' . $this->field[ 'name' ] . '[title][]' . $this->field['name_suffix'] .'">' . sprintf ( __ ( 'Add %s', 'atelier' ), $this->field[ 'content_title' ] ) . '</a><br/>';
        }

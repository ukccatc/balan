...
class jckqv {
	...
	private function define_constants() {
		...
		$this->define( 'ICONIC_WQV_IS_ENVATO', true );
	...
	private function load_classes() {
		...
		$this->templates = new Iconic_Template_Loader( $this->slug, 'jck-woo-quickview', ICONIC_WQV_PATH );
		Iconic_WQV_Licence::run();
	...
	public function load_settings_framework() {
		$settings_framework = new Iconic_WQV_Settings( ICONIC_WQV_PATH . 'inc/admin/settings.php', 'jckqv', $this->name, $this->short_name );
		$this->settings     = $settings_framework->settings;
	}
	...
	public function sanitize_settings( $settings ) {
		...
		// Validate Margins
		$i = 0;
		foreach ( $settings['trigger_position_margins'] as $marVal ) {
	...
	public function modal() {
		...
		$classes = apply_filters( 'jck_qv_modal_classes', array(
			'cf',
			'product',
	...
	public function register_scripts_and_styles() {
		...
		if ( ! is_admin() ) {
			...
			$this->load_file( $this->slug . '-script', '/assets/frontend/js/main.min.js', true, array(
				'jquery',
				'jquery-effects-core',
				'wp-util',
			) );

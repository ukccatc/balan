 	...
 	SPB.general = {
 		...
 		initFullWidthElements: function() {
 			...
 			jQuery('.spb-full-width-element').each(function() {
				...
				if ( SPB.var.body.hasClass('bootstrap-4') ) {
	 				element.parent().addClass('has-fw-elements');
	 				element.parent().addClass('container-fluid bs-4-row');
 		...
 		initRows: function() {
 			...
 			jQuery('.spb-row').each(function() {
 				...
 				if ( spbRow.isFullWidth && SPB.var.body.hasClass('bootstrap-4') ) {
 					if ( element.data('wrap') === "full-width-contained" ) {
						element.parent().addClass('container-fluid bs-4-row');
						element.addClass('container');
					} else if ( element.data('wrap') === "full-width-stretch" ) {
						element.parent().addClass('container bs-4-row');
					}
	 				spbRow.isFullWidth = false;
	 			} else if (  SPB.var.body.hasClass('bootstrap-4') ) {
 					element.parent().addClass('container bs-4-row');
 				}
